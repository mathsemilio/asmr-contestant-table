...
class FetchWeekHighlightsUseCase(private val endpoint: WeekHighlightsEndpoint) :
        ...
        endpoint.fetchWeekHighlights().also { result ->
            when (result) {
                is Result.Completed -> notifyListener {
                    it.onWeekHighlightsFetchedSuccessfully(result.data!!)
                }
                is Result.Failed -> notifyListener { it.onWeekHighlightsFetchFailed() }
            }
        }
    }
}